...
export async function POST(request: Request, ctx: { params: Promise<{ id: string; step: string }> }) {
  try {
    ...
    if (stepKey === "new_hire_checklist") {
      const data = mergeNewHireChecklistData(body.data);
      const markCompleted = body.markCompleted === true;
      if (markCompleted) {
        // Check that every other applicable step is in a finished state.
        const steps = applicableSteps({ desiredRole: application.desiredRole, isExistingEmployee: false });
        const progress = await getIntakeProgress(application.id, { desiredRole: application.desiredRole, isExistingEmployee: false });
        const upstream = progress.map((p) => ({ def: p.def, status: p.status }));
        void steps;
        const errors = validateNewHireChecklistForCompletion(data, upstream);
        if (errors.length) return NextResponse.json({ error: errors[0] }, { status: 400 });
      }
      const sigName = data.signatureName.trim() ? data.signatureName.trim() : null;
      await saveIntakeStepData({

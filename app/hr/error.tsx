  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-gradient-to-br from-orange-50 via-white to-amber-50 px-4 py-12">
      ...
      <div className="relative w-full max-w-lg rounded-3xl border border-orange-100 bg-white/95 p-8 shadow-2xl">
        <p className="inline-flex rounded-full bg-orange-100 px-3 py-1 text-xs font-bold uppercase tracking-wide text-orange-700">HR Operations</p>
        <h1 className="mt-4 text-2xl font-semibold tracking-tight text-slate-950">This page didn&apos;t load.</h1>
        <p className="mt-2 text-sm text-slate-700">
          The error has been logged. Try again, or jump back to your dashboard. If this is blocking you, refresh once more — sometimes a stale session is the cause.
        </p>
        {error?.message && (
          <details className="mt-3 rounded-md border border-slate-200 bg-slate-50 p-3 text-xs">
            <summary className="cursor-pointer font-medium text-slate-700">Show technical details</summary>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          <Button asChild variant="outline"><Link href="/hr/dashboard"><ArrowLeft size={16} /> HR dashboard</Link></Button>
        </div>
      </div>
    </main>
  );

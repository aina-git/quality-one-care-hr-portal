  ...
  return (
    <DashboardShell user={user} nav={[{ href: "/hr/dashboard", label: "Dashboard" }, { href: "/hr/applications", label: "Applications" }, { href: "/hr/training", label: "Training" }]}>
      <div className="grid gap-6">
        ...
        {tab === "summary" && <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]"><Card><CardHeader><CardTitle>Summary</CardTitle></CardHeader><CardContent className="grid gap-2 text-sm"><p>Role: {application.desiredRole ?? "Not recorded"}</p><p>Submitted: {application.submittedAt?.toLocaleString() ?? "Not submitted"}</p><p>Phone: {application.applicantProfile.phone ?? "Not recorded"}</p><p>Email: {application.applicantProfile.user.email}</p><p>Next action: {progress?.nextActionRequired ?? "Monitor workflow."}</p><div className="flex flex-wrap gap-2 pt-3"><Link className="rounded-md bg-orange-600 px-3 py-2 text-sm font-medium text-white" href={`/hr/applications/${id}/review`}>Open Screening Review</Link><Link className="rounded-md border px-3 py-2 text-sm font-medium" href={`/hr/applications/${id}/verification`}>Final Verification</Link><Link className="rounded-md border px-3 py-2 text-sm font-medium" href={`/hr/applicants/${application.applicantProfileId}`}>Applicant Profile</Link></div>{progress ? <div className="pt-3"><ApplicantProgressTimeline stages={progress.stages} compact /></div> : null}</CardContent></Card><Card><CardHeader><CardTitle>Workflow Controls</CardTitle></CardHeader><CardContent>{canEdit ? <ApplicationWorkflowControls applicationId={application.id} currentStatus={application.status} /> : <p className="text-sm text-muted-foreground">Read-only access.</p>}</CardContent></Card></div>}
        ...
        {tab === "calendar" && <Card><CardHeader><CardTitle>Calendar</CardTitle></CardHeader><CardContent className="grid gap-4">{canEdit ? <CalendarEventForm applicationId={id} applicantUserId={application.applicantProfile.userId} /> : null}{application.calendarEvents.map((event) => <div key={event.id} className="rounded-md border bg-blue-50 p-3 text-sm"><p className="font-medium">{event.title}</p><p>{event.startDateTime.toLocaleString()} to {event.endDateTime.toLocaleString()}</p><p className="mt-1 text-xs">{label(event.eventType)} - {label(event.status)}</p></div>)}</CardContent></Card>}

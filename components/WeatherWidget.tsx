...
async function fetchWeather(zip: string): Promise<WeatherPayload> {
  const cleaned = zip.trim();
  if (!/^\d{5}$/.test(cleaned)) {
    throw new Error("Enter a valid 5-digit US ZIP code.");
  }
  const response = await fetch(`/api/weather?zip=${encodeURIComponent(cleaned)}`);
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(payload.error ?? `Weather fetch failed (${response.status}).`);
  }
...
export function WeatherWidget() {
  ...
  async function load(zip: string, persist: boolean) {
    ...
    setError(null);
    try {
      const payload = await fetchWeather(zip);
      setData(payload);
      setZipInput(payload.location.zip);
      if (persist && typeof window !== "undefined") {
        window.localStorage.setItem(STORAGE_KEY, payload.location.zip);
      }
    } catch (cause) {
